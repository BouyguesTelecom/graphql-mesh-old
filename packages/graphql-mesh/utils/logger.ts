/**
 * Logger: Static Class
 * Used to log events with contextual information and useful data
 *
 * TODO: Split into specific loggers and core logger
 */
import { v4 as uuidv4 } from 'uuid'

export class Logger {
  private static level: string = process.env['LogLevel'] || 'INFO'
  private static format: string = process.env['LogFormat'] || 'HUMAN' // Set to JSON_STRING for one-line JSON string logs
  private static bodyMaxLogSize = process.env['LogBodyMaxSize']
    ? parseInt(process.env['LogBodyMaxSize'])
    : 200
  private static maxStackLogSize = process.env['LogStackTraceMaxSize']
    ? parseInt(process.env['LogStackTraceMaxSize'])
    : 500
  private static envLog: string = process.env['LogEnvFieldToAdd'] // Add extra env fields in JSON log e.g., "app=graphql,env.name=production,env.site=Paris"
  private static localDateCountry: string = process.env['LogLocalDateCountry'] || 'fr-FR'
  private static logHeaders = defineLogHeaders(
    process.env['LogHeaders'] ||
      'headers.host=host,headers.origin,headers.user-agent=user-agent,headers.content-length=content-length,headers.authorization=authorization'
  )
  private static logContextHeaders = defineLogHeaders(
    process.env['LogContextHeaders'] || 'ctx.requestId=x-request-id'
  )
  private static skipHealthCheck = process.env['skipHealthCheck']
    ? process.env['skipHealthCheck'] == 'true'
    : true
  private static healthCheckHeaderName = process.env['healthCheckHeader'] || 'x-internal'
  private static healthCheckHeaderValue = process.env['healthCheckHeaderValue'] || 'health-check'
  private static maxErrorStackSize = process.env['MaxErrorStackSize']
    ? parseInt(process.env['MaxErrorStackSize'])
    : 200

  constructor() {}

  /**
   * Core logger with human-readable format or machine-friendly one-line JSON string format
   */
  private static log(
    level: string,
    typeEv: string,
    ...
    if (Logger.format == 'JSON_STRING') {
      ...
      try {
        ...
        // If exception error, add message and a portion of the stack trace
        if (err) {
          log.events[0]['exception'] = {
            message: err.message,
            stack: err.stack?.substring(0, this.maxErrorStackSize) + ' ...'
          }
        }
        // If defined, add extra env fields
        if (this.envLog) {
          addEnvFieldLog(this.envLog, log)
        }
      } catch (e) {
        log['error'] = 'Error while getting log event contextual info: ' + e.message
      }
  ...
  public static warn(
    ...
    if (Logger.level == 'WARN' || Logger.level == 'INFO' || Logger.level == 'DEBUG') {
      Logger.log('WARN', typeEv, source + ': ' + message, headers, false, info, e)
    }
  }
  ...
  public static endExec(headers: any, result: any, duration: number, resultLogInfo: any) {
    try {
      if (this.isEventToLog(headers)) {
        const info = {
          result: InfoResult(result, this.maxStackLogSize, resultLogInfo),
          duration: duration
        }
  ...
  /**
   * Log the onRequestParseDone event
   */
  public static onRequestParseDone(
    headers: any,
    query: any,
    operation: string,
    variables: any,
    ...
    if (this.isEventToLog(headers)) {
      ...
      Logger.log('INFO', 'requestParseDone', 'Request Parse', headers, true, info)
    }
  }

  /**
   * Log the onResponse event when HTTP response returns
   */
  ...
  public static onRequest(request: any) {
    ...
    try {
      if (this.isEventToLog(headers)) {
        ...
        Logger.log('INFO', 'onRequest', 'Request incoming', headers, false, info)
      }
    } catch (e) {
      Logger.error(
        'LOGGER_ERROR',
  ...
  private static addHeadersToContextLog(log: any, headers: any) {
    try {
      ...
      Logger.error(
        'LOGGER_ERROR',
        'onHeadersToLog',
        'Error during headers log generation',
        headers,
        null,
        e
      )
...
/**
 * Used to add some environment fields to the log, like env.name, app.name, etc.
 * Example: logEnvInfoField: string = "app=graphql,env.name=production,env.site=Paris"
 * -> log={
 *          app: "graphql",
 *          env: {
 *             name: "production",
...
function addEnvFieldLog(logEnvInfoField: string, log: any) {
  // Split fields list with dot separator
  ...
  for (let idxField = 0; idxField < fields.length; idxField++) {
    ...
    let current = log
    // Create new object if it does not exist for each level and set value for leaf level
...
function defineLogHeaders(logHeaders: string) {
  ...
  if (logHeaders) {
    ...
    for (const headerKey in headers) {
      ...
      headersList.push({
        name: headerInfo[0],
        header: headerInfo[1] ? headerInfo[1] : headerInfo[0]
      })
    }
  }
  return headersList

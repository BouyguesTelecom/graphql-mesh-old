  ...
  Object.entries(schemas).forEach(([schemaKey, schemaValue]) => {
    Object.entries(schemaValue)
      ...
      .forEach(([, schema]) => {
        ...
        const objToExtend = Object.keys(interfacesWithChildren).includes(trimedSchemaKey) ? 'interface' : 'type'
        ...
        if (objToExtend === 'interface' && typeDefs !== '' && resolvers[trimedSchemaKey] !== undefined) {
          ...
          resolvers[trimedSchemaKey].__resolveType = (res, _, schema) => {
            if (res.__typename !== undefined) {
              ...
              if (schema.parentType._fields[schema.fieldName] !== undefined) {
                //TODO:
                return interfacesWithChildren[schema.parentType._fields[schema.fieldName].type.name][interfacesWithChildren[schema.parentType._fields[schema.fieldName].type.name].length-1]
              }
              else {
